...
## 2025-05-14 - Communicating Form Errors with aria-invalid
**Learning:** For forms using asynchronous validation (like Formspree), merely displaying error messages is insufficient for screen readers. Dynamically setting the `aria-invalid` attribute on inputs based on the validation state ensures that assistive technologies can immediately announce the invalid state of a field to the user.
**Action:** Always synchronize the `aria-invalid` attribute with the form's error state for every input field.
